...
export function generateApiNamespacesSource(config, apiContext) {
  return [
    ...
    `export const GENERATED_STANDALONE_APPS = ${JSON.stringify(collectStandaloneApps(config), null, 2)} as const;`,
    '',
    `export const GENERATED_FEDERATION_DEV_ENTRIES = ${JSON.stringify(collectFederationDevEntries(config), null, 2)} as const;`,
    '',
  ].join('\n');
}
...
export function buildWindowConfigArtifacts(rootDir) {
  ...
  const config = loadJsonFile(paths.configPath);
  ...
  const errors = validateWindowsConfig(config, schema, {
