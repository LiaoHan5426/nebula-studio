...
import type { EditorError, EditorOptions } from './types';
...
export const EditorInstanceManager = {
  register(id: string, instance: EditorInstance): void {
    editorInstances.set(id, instance);
  },

  get(id: string): EditorInstance | undefined {
 ...
 */
export const registerLanguageSupport = (
  languageId: string,
...
export const isMonacoAvailable = (): boolean => {
  ...
  return (
    typeof monacoWindow.monaco !== 'undefined' &&
    typeof monacoWindow.monaco.editor !== 'undefined'
  );
};
 ...
 * @param options 编辑器选项
 */
export const validateEditorOptions = (options: unknown): EditorError | null => {
  if (!options || typeof options !== 'object') {
    return createEditorError(
      'INVALID_OPTIONS',
      'Editor options must be an object',
    );
  }

  const editorOptions = options as EditorOptions;

  if (
    editorOptions.value !== undefined &&
    typeof editorOptions.value !== 'string'
  ) {
    return createEditorError('INVALID_VALUE', 'Editor value must be a string');
  }

  if (
    editorOptions.language !== undefined &&
    !isSupportedLanguage(editorOptions.language)
  ) {
    return createEditorError(
      'UNSUPPORTED_LANGUAGE',
      `Language "${editorOptions.language}" is not supported`,
      undefined,
      true,
    );
  }
