...
class WindowStateManager {
  ...
  constructor(options: WindowStateManagerOptions = {}) {
    ...
    this.fullStoreFileName = join(this.config.path, this.config.file);
  ...
  public saveState(win?: BrowserWindow): void {
    ...
    try {
      ...
      const dir = dirname(this.fullStoreFileName);
      if (!existsSync(dir)) {
  ...
  private findBestDisplay(): Electron.Display | null {
    ...
    if (this.state.displayId) {
      const displayById = displays.find(
        (display) => display.id === this.state.displayId,
      );
      if (displayById) {
        return displayById;
      }
    ...
    const displayBounds = this.state.displayBounds;
    if (displayBounds) {
  ...
  private loadState(): void {
    try {
      const data = readFileSync(this.fullStoreFileName, 'utf8');
