...
| Renderer 样式 | `src/renderer/styles/electron-overrides.css` | **仅桌面** 的样式覆盖；全局设计链走 `@nebula-studio/styles/document`（在 `boot.ts` 引入） |
...
3. **Vite 配置**
   - `electron.vite.config.ts` → `@nebula-studio-internal/build-kit` 的 `defineNebulaConfig({ platform: 'electron' })`。
