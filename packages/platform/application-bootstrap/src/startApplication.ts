...
import type { StartApplicationOptions } from './index';
...
let activeHandle: ApplicationHandle | null = null;

/**
 * 统一微应用启动入口（兼容 adapter）。
 *
 * Federation Remote 使用 `NebulaRemoteApplication`（application-contract），不要新增 startApplication 调用方。
 *
 * 内部启动顺序：
 ...
 *
 * 返回 ApplicationHandle，调用 dispose() 可释放 auth 监听、event bus 订阅与 Vue 实例。
 */
...
async function runAuth(
  ...
  if (options.auth?.enabled) {
    try {
      const { AuthBootstrap } =
        await import('@nebula-studio/auth-provider/bootstrap');
      ...
      console.error('[application-bootstrap] AuthBootstrap failed:', error);
      return false;
    }
  }
...
export function resetActiveApplicationHandleForTests(): void {
  if (activeHandle) {
    try {
