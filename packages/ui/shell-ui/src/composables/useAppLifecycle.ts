...
export interface UseAppLifecycleOptions {
  extraAvailableViewIds?: () => string[];
  getAuthSession: () => null | ShellAuthSessionPayload;
  isExternalView?: (viewId: string) => boolean;
  layoutPreferences: {
    accentPreset: string;
    collapsed: boolean;
    themeMode: string;
  };
  openExternalView?: (viewId: string) => void;
  openLogin: () => Promise<void>;
  refreshAuthSession: () => Promise<void>;
  resetOrganizationSession: () => void;
  resolveIframeSrc?: (viewId: string) => string | undefined;
...
export function useAppLifecycle(opts: UseAppLifecycleOptions) {
  ...
  async function switchEmbeddedView(viewId: string): Promise<void> {
    ...
    if (opts.isExternalView?.(viewId)) {
      opts.openExternalView?.(viewId);
      return;
    }
    if (opts.resolveIframeSrc?.(viewId)) {
      activeViewId.value = viewId;
      syncSidebarSelection(viewId);
      return;
    }
    if (!usesIframeEmbed) reportShellViewport();
    const ok = await resolveRendererIpc().invoke('shell:set-active-view', {
      viewId,
    });
    if (ok) {
      activeViewId.value = viewId;
  ...
  async function enableEmbeddedView(viewId: string): Promise<boolean> {
    return Boolean(
      await resolveRendererIpc().invoke('shell:enable-embedded-view', {
        viewId,
      }),
    );
  }
  async function disableEmbeddedView(viewId: string): Promise<boolean> {
    return Boolean(
      await resolveRendererIpc().invoke('shell:disable-embedded-view', {
        viewId,
      }),
    );
  }
  async function reorderEmbeddedViews(ids: string[]): Promise<boolean> {
    return Boolean(
      await resolveRendererIpc().invoke('shell:reorder-embedded-views', {
        orderedViewIds: ids,
      }),
    );
  }
  ...
  function reportShellViewport(): void {
    ...
    resolveRendererIpc().invoke('shell-viewport', {
      x: rect?.left ?? 0,
  ...
  async function loadShellState(): Promise<void> {
    const [state, nextTheme, nextMode] = (await Promise.all([
      resolveRendererIpc().invoke('shell:get-state'),
      resolveRendererIpc().invoke('settings:theme:get'),
      resolveRendererIpc().invoke('shell:app-mode:get'),
    ...
    availableViewIds.value = Array.isArray(state?.availableViewIds)
      ...
      : [];
    const extras = opts.extraAvailableViewIds?.() ?? [];
    availableViewIds.value = [
      ...availableViewIds.value,
      ...extras.filter((id) => !availableViewIds.value.includes(id)),
    ];
  ...
  async function applyTheme(nextTheme: ThemeMode): Promise<void> {
    ...
    theme.value = (await resolveRendererIpc().invoke('settings:theme:set', {
      theme: nextTheme,
    })) as ThemeMode;
    setTimeout(() => {
   ...
   * 独立侧边栏应用（`integratable: false`）与集成应用均来自 registry / chrome catalog。
   */
  ...
  const onAuthSessionChanged = (_e: unknown, ...args: unknown[]): void => {
    const p = (args[0] ?? null) as null | ShellAuthSessionPayload;
