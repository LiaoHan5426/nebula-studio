...
export interface UseAppIntegrationOptions {
  ...
  shellHost: ReturnType<
    typeof import('@nebula-studio/shell-protocol').getShellHostBridge
  >;
  /** 顶部像素偏移 */
  shellTopPx: number;
  /** 可排序视图 ID 列表（响应式） */
...
export function useAppIntegration(opts: UseAppIntegrationOptions) {
  ...
  function syncShellEmbeddedContentVisible(): void {
    ...
    resolveRendererIpc().invoke('shell:set-embedded-content-visible', {
  ...
  async function openWorkspace(): Promise<void> {
    ...
    if (!opts.usesIframeEmbed) {
      ...
      await resolveRendererIpc().invoke('shell:clear-active-embedded-view');
    }
  }
