...
function createHandlers() {
  return createBridgeHandlers({
    ...
    externalLinkService: {
      openExternal: vi.fn(async () => ({ success: true as const })),
    },
    fileService: {
...
describe('bridge/main/dispatch', () => {
  it('returns runtime info for valid runtime.getInfo request', async () => {
    ...
    if (response.ok) {
      expect((response.data as RuntimeInfo).protocolVersion).toBe(
        BRIDGE_PROTOCOL_VERSION,
      );
  ...
  it('returns timeout when handler exceeds timeout budget', async () => {
    ...
    const dispatch = createMainDispatcher(slowHandlers, {
      defaultTimeoutMs: 1,
    });
    const response = await dispatch({
      protocolVersion: BRIDGE_PROTOCOL_VERSION,
